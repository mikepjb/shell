...
-- Navi Integration -----------------------------------------------------------
local navi_state = { buf = nil, win = nil }

-- Clean up state when navi buffer is closed
vim.api.nvim_create_autocmd("BufDelete", {
  callback = function(ev)
    if navi_state.buf == ev.buf then
      navi_state.buf = nil
      navi_state.win = nil
    end
  end,
...
local function navi_toggle()
  if not navi_state.buf or not vim.api.nvim_buf_is_valid(navi_state.buf) then
    return vim.notify("No navi buffer to toggle")
  end

  if navi_state.win and vim.api.nvim_win_is_valid(navi_state.win) then
...
local function navi_query(with_selection)
  vim.ui.input({ prompt = "navi: " }, function(prompt)
    ...
    if with_selection then
      ...
      input = input .. "\n\n-- " .. filepath .. " (" .. filetype .. "), line " .. cursor_line .. "\n"
      input = input .. "```" .. filetype .. "\n" .. code .. "\n```"
    else
      input = input .. "\n\n[" .. filepath .. ", " .. filetype .. ", line " .. cursor_line .. "]"
    end

    -- Only delete old buffer when actually creating a new query
    if navi_state.buf and vim.api.nvim_buf_is_valid(navi_state.buf) then
      vim.api.nvim_buf_delete(navi_state.buf, { force = true })
    end
...
vim.keymap.set("n", "<M-z>", navi_toggle, { silent = true })
